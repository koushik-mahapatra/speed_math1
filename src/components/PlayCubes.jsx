...
const PlayCubes = () => {
  ...
  // Generate a random cube question with multiple options within the specified range
  const generateQuestion = () => {
    const num =
      Math.floor(Math.random() * (parseInt(endNum) - parseInt(startNum) + 1)) +
      parseInt(startNum);
    const correctAnswer = num * num * num; // Cube of the number
    setQuestion(`${num}³ = ?`);
